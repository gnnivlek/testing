...
RUN  wget --no-check-certificate -e use_proxy=yes -e https_proxy=10.81.58.18:8080 -e http_proxy=10.81.58.18:8080 --no-cookies --header "Cookie: eula_3_1_agreed=tools.hana.ondemand.com/developer-license-3_1.txt; path=/;" -S https://tools.hana.ondemand.com/additional/sapcc-$SAPCC_VERSION-linux-x64.zip && \
    wget --no-check-certificate -e use_proxy=yes -e https_proxy=10.81.58.18:8080 -e http_proxy=10.81.58.18:8080 --no-cookies --header "Cookie: eula_3_1_agreed=tools.hana.ondemand.com/developer-license-3_1.txt; path=/;" -S https://tools.hana.ondemand.com/additional/sapjvm-$SAPJVM_VERSION-linux-x64.rpm && \
...
CMD /opt/sapjvm_8/bin/java \
  ...
  -Dpath.to.runtime.installation.folder=/opt/sap/scc \
  -Dserver.config.home=/opt/sap/scc \
  ...
  -Dcatalina.base=/opt/sap/scc \
  -Dcatalina.home=/opt/sap/scc \
  -Djava.io.tmpdir=/opt/sap/scc \
  -Djava.util.logging.config.file=/opt/sap/scc/conf/logging.properties \
  -Djava.class.path=/opt/sap/scc/bin/jul-to-slf4j.jar:/opt/sap/scc/bin/slf4j-api.jar:/opt/sap/scc/bin/logback-classic.jar:/opt/sap/scc/bin/logback-core.jar:/opt/sap/scc/bin/com.sap.core.js.logging.jar:/opt/sap/scc/bin/com.sap.scc.t8logback.jar:/opt/sap/scc/lib/sysout-over-slf4j.jar:/opt/sap/scc/config_master/com.sap.js.logging/:/opt/sapjvm_8/lib/jvmx.jar:/opt/sapjvm_8/lib/tools.jar:/opt/sap/scc/bin/bootstrap.jar:/opt/sap/scc/bin/tomcat-juli.jar com.sap.scc.tomcat.utils.SCCBootStrap
